...
export default async function LezenPage({
  ...
  const lessons = await getLessonsForSkill(targetLevel, "lezen");
  const fullAccess = hasFullAccess(profile);

  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-4">
      <h1 className="text-2xl font-bold text-navy-900">{t("lezenTitle")}</h1>
      {isAdmin ? <LevelSwitcher basePath="/lezen" activeLevel={targetLevel} /> : null}
      {lessons.length === 0 ? (
